...
@section('content')
    <div class="d-flex align-items-center justify-content-between mb-4 flex-wrap gap-2">
        <div>
            <h2 class="fw-bold mb-1">Blog Categories</h2>
            <p class="text-muted mb-0">Manage the categories used to organize and filter blog posts.</p>
        </div>
        <a href="{{ route('admin.blog-categories.create') }}" class="btn btn-admin-gradient">
    ...
    <div class="admin-card">
        <div class="admin-card__header">
            <form action="{{ route('admin.blog-categories.index') }}" method="GET" class="d-flex gap-2">
        ...
        <form id="bulkDeleteForm" action="{{ route('admin.blog-categories.bulk-destroy') }}" method="POST">
            @csrf
            @method('DELETE')
                    ...
                    <tbody>
                        @forelse($categories as $category)
                            <tr>
                                ...
                                <td><span class="admin-badge admin-badge--info">{{ $category->posts_count }}</span></td>
                                ...
                                <td class="text-end">
                                    <a href="{{ route('admin.blog-categories.edit', $category) }}" class="btn-admin-icon btn-admin-icon--edit"><i class="bi bi-pencil"></i></a>
                                    <button class="btn-admin-icon btn-admin-icon--delete" type="button" data-confirm-delete="{{ $category->name }}" data-action="{{ route('admin.blog-categories.destroy', $category) }}"><i class="bi bi-trash3"></i></button>
                                </td>
                            </tr>
                        @empty
        ...
        @foreach($categories as $category)
            <form id="toggle-category-{{ $category->id }}" action="{{ route('admin.blog-categories.toggle-status', $category) }}" method="POST" class="d-none">
                @csrf @method('PATCH')
            </form>
        @endforeach
