...
@section('content')
    <div class="d-flex align-items-center justify-content-between mb-4 flex-wrap gap-2">
        <div>
            ...
            <p class="text-muted mb-0">{{ $messages->total() }} message{{ $messages->total() === 1 ? '' : 's' }} total — {{ $unreadCount }} unread.</p>
        </div>
    </div>
                    ...
                    <tbody>
                        @forelse($messages as $message)
                            <tr class="{{ $message->is_read ? '' : 'fw-semibold' }}">
                                <td><input type="checkbox" name="ids[]" value="{{ $message->id }}" class="form-check-input row-check"></td>
                                <td>
                                    <a href="{{ route('admin.messages.show', $message) }}" class="text-decoration-none text-reset">
                                        {{ $message->name }}
                                        <div class="text-muted small fw-normal">{{ $message->email }}</div>
                                    </a>
                                </td>
                                <td class="text-muted">{{ $message->service_interested ?: '—' }}</td>
                                <td class="text-muted fw-normal">{{ \Illuminate\Support\Str::limit($message->message, 60) }}</td>
                                <td class="text-muted fw-normal">{{ $message->created_at->diffForHumans() }}</td>
                                <td>
                                    @if($message->is_read)
                                    ...
                                    @if($message->replied_at)
                                        <span class="admin-badge admin-badge--success"><i class="bi bi-reply-fill"></i> Replied</span>
                                    @endif
                                </td>
                                <td class="text-end">
                                    <a href="{{ route('admin.messages.show', $message) }}" class="btn-admin-icon btn-admin-icon--edit"><i class="bi bi-eye"></i></a>
                                    <button class="btn-admin-icon btn-admin-icon--delete" type="button" data-confirm-delete="the message from {{ $message->name }}" data-action="{{ route('admin.messages.destroy', $message) }}"><i class="bi bi-trash3"></i></button>
                                </td>
                            </tr>
                        @empty
